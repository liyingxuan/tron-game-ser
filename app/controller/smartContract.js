...
class SmartContractController extends Controller {
	...
	async create() {
		...
		if (typeof(ctx.request.body.address) === 'undefined' || ctx.request.body.address === '') {
			...
			let modulo = typeof(ctx.request.body.modulo) === 'undefined' ? '' : ctx.request.body.modulo.toString();
			let value = typeof(ctx.request.body.value) === 'undefined' ? '' : ctx.request.body.value.toString();
